...
export async function POST(req: Request) {
  try {
    ...
    const systemMessage: { role: MessageRole, content: string } = {
      role: 'system',
      content: systemMessageContent || 'You are skilled at extracting information and providing concise answeers. Ensure you provide your response in the format: "category": ["name", "name"]'
    };

    const extractTask: { role: MessageRole, content: string } = {
      role: "user",
      content: `Review the following text. Identify all the ${category} in the text below \n\n Text: ${text} Provide your response ONLY as a dictionary list: 'category': ['name', 'name']`
    };
