...
export async function POST(
  ...
  try {
    ...
    if (!promptId) {
      return new NextResponse("Prompt id is required", { status: 401 });
    }

    if (!content) {
      return new NextResponse("Content is required", { status: 402 });
    }
    ...
    if (!sessionByUserId) {
      return new NextResponse("Invalid session or user", { status: 400 });
    }

    // Create a new response associated with the given promptId
